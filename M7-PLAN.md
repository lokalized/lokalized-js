...
**Verification.** Four discriminating pairs; passing one half proves nothing:
- `owed-ds.heuristic-depth-outranks-weight` vs `.heuristic-depth-agrees-with-weight` — same ranges, swapped weights, different answer. Catches a range-major loop where Java is category-major.
- `m3b-negotiation.zero-weight-exact-anchor-does-exclude` vs `.zero-weight-heuristic-anchor-does-not-exclude` — dropping the weight condition from `broadPositiveStructuralRange`/`broadPositiveSemanticRange` flips both.
